...
float wave(vec3 position) {
  return fit(smoothMod(position.y * 5.0, 1.0, 1.0), 0.35, 0.6, 0.0, 1.0);
}
...
vec3 coords = normal;
  ...
  //
  float displacement = vDisplacement / u_data;
    vec3 newPosition = position + normal * displacement;
    vec4 modelViewPosition = modelViewMatrix * vec4(newPosition, 1.5);
